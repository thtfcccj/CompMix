                          ...
                          unsigned short x, unsigned short y)//绝对x,y
{
  ...
  unsigned short cw = pNsDesc->Progress.w>> 12;
  unsigned short ch = pNsDesc->Progress.h>> 12;
  
   //不是实时刷新时，画边框  
  Color_t PrvBg = Plot_GetBrushStyle(); //用后需恢复
  if(!(Info & 0x80) && (pNsDesc->Progress.cBg != pNsDesc->Progress.cRect)){
    Plot_SetBrushStyle(pNsDesc->Progress.cRect);
  ...
  y += ch;
  h -= ch * 2;
  Plot_SetBrushStyle(pNsDesc->Progress.cFg[pNsPara->Progress.FgId & 0x03]);//前景
  if(Info & 0x10){//纵向进度条
    ch = pNsPara->Progress.Percent * 100 / h;
    if(ch > 100) ch = 100;//超限了
    Plot_FullRect(x, y + h - ch, w, ch); //进度
    Plot_SetBrushStyle(pNsDesc->Progress.cBg);//背景
   ...
   Plot_SetBrushStyle(PrvBg);//恢复原有背景  
}
