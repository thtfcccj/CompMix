...
//------------------------关于定时器2寄存器的转义------------------------
  ...
  #define PICB_TMRxON   PICB_TMR2ON  //控制
  #define PRx          PR4           //分频
...
static void _StartPWM(void)
{
 //输出频率 = _MCU_HZ / ((预分频值(4bit) * (分频器值(8bit))
  ...
  CCPxCON  =  PICB_CCP3M2 | PICB_CCP3M3;//PWM模式P2A出，高电平有效。占空比高两位
  TxCON = PICB_TMR4ON | PERIOD_PWM_GEN_PS; //使用最高Fosc/4(8MHZ)以实现频率的细微调整
}
//---------------------------停止PWM函数-------------------------------------
